...
impl Vec2 {
    ...
    fn sum(&self,o:Vec2)->Vec2 {
        Vec2::new(self.x + o.x, self.y + o.y)
    }

    fn new_dir(move_dir:i128)->Vec2 
    {
...
impl Field{
    ...
    fn oxygen(&self, s:Vec2)->i64
    {
        ...
        while positions.len()>0 {
            ...
            if !v {
                res = std::cmp::max(res,l);
                was.insert(pos,true);
                
                let moves = vec![
                 pos.sum(Vec2::new_dir(1)),
                 pos.sum(Vec2::new_dir(2)),
                 pos.sum(Vec2::new_dir(3)),
                 pos.sum(Vec2::new_dir(4))
                ];

                for dir in moves {
                  if !was.get(&dir).unwrap_or(&false) && self.is_ok(dir) { positions.push_back((dir,l+1)); }
                }
            }
        }
...
fn main() {
    ...
    for io in 0..=4000000
    {
        ...
        let new_pos = pos.sum(Vec2::new_dir(move_dir));
    ...
    println!("oxygen time:{}",f.oxygen(end_pos.unwrap()));
}
