fn len(a:(i32,i32),b:(i32,i32)) -> i32
{
...
fn draw(f:&mut Vec<Vec<u32>>,s:String,pos:&mut(i32,i32),start_point:(i32,i32),check:bool) -> u32
{
    let cnt = s[1..].to_string().parse::<i32>().unwrap();
    let ss = s.chars().next().unwrap();
    let mut res = 10000i32;

    let moved : (i32,i32) = match ss {
      'L' => (-1, 0),
      'U' => ( 0,-1),
      'R' => ( 1, 0),
      'D' => ( 0, 1),
    ...
    for _i in 0..cnt {
      ...
      if check && f[pos.1 as usize][pos.0 as usize]==1 {
        let l = len(start_point,(pos.0,pos.1));
        if l<res {
            res = l;
        }
      }
...
fn comp(w1: Vec<&str>,w2:Vec<&str>) -> u32 {
  ...
  let mut field = vec![vec![0u32;size*2];size*2];

  let mut pos = (size as i32,size as i32);
  let start_point = (size as i32,size as i32);

  for wire1 in w1 {
    draw(&mut field,wire1.to_string(),&mut pos,start_point,false);
  } 

  //draw_debug(&mut field);
  
  pos = (size as i32,size as i32);
  let mut min_len = size as u32;

  for wire2 in w2 {
    let l = draw(&mut field,wire2.to_string(),&mut pos,start_point,true);
    if l<min_len {
      min_len = l;
    }
  } 
