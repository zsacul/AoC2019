...
fn draw(f:&mut Vec<Vec<u8>>,s:String,pos:&mut(i32,i32),check:bool) -> Vec<(usize,usize)>
{
  let move_dir = get_dir(s.chars().next().unwrap());
  let mut vv = vec![];
  ...
  for _i in 0..cnt {
    ...
    if !check  { f[p.1][p.0]=1; }
    if  check && f[p.1][p.0]==1 { vv.push((p.0,p.1)); }
  }
  vv
}

fn min_route(field: Vec<Vec<u8>>,route:&Vec<&str>,start_point:(i32,i32)) -> HashMap<(usize,usize),u32>
{
  let mut hash: HashMap<(usize,usize),u32> = HashMap::new();
  let mut dist = 0;
  let mut pos: (i32,i32) = start_point.clone();
  ...
  for s in route {
      ...
      for _i in 0..cnt {
        ...
        pos.1+=move_dir.1;
        dist+=1;
        let pp = (pos.0 as usize,pos.1 as usize);
...
fn fill_intersections(field:&mut Vec<Vec<u8>>,cross:&Vec<(usize,usize)>)
{
  for p in cross { field[p.1][p.0] = 2; } 
}

fn comp(w1: Vec<&str>,w2:Vec<&str>) -> u32 {
  let size        = 6*2048 as usize;
  ...
  for wire2 in &w2 {
    points.append(&mut draw(&mut field,wire2.to_string(),&mut pos,true));
  }
  ...
  fill_intersections(&mut field1,&points);
  fill_intersections(&mut field2,&points);

  let hash1 = min_route(field1,&w1,start_point);
  let hash2 = min_route(field2,&w2,start_point);

  let mut minv = std::u32::MAX;

  for p in points {
    minv = min(minv,hash1[&p] + hash2[&p])
  }
